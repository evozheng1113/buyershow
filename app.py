...
REVIEW_MIX = [
    ...
    ("尺寸/上手效果", 2, "尺寸合适、显大、50分日常刚好、圈号量准了"),
    ("做工细节", 2, "做工精致、不刮手、镶嵌牢固、打磨圆润、无瑕疵"),
    ("性价比", 1, "性价比超高、同参数天然钻要五位数、双证"),
    ("送礼/场景", 1, "女朋友喜欢、求婚成功、包装礼盒、通勤约会都合适"),
    ("服务/售后", 1, "客服耐心、免费改圈、支持复检、物流很快"),
]


def _mix_for(n):
    """按 10 条的标准配比,等比缩放到实际条数 n(至少各留 1 条给闪度和尺寸)。"""
    out, total = [], sum(c for _, c, _ in REVIEW_MIX)
    for name, cnt, words in REVIEW_MIX:
        k = max(1, round(cnt * n / total)) if cnt >= 2 else round(cnt * n / total)
        if k > 0:
            out.append(f"- {name}:{k} 条(自然出现这些词:{words})")
    return "\n".join(out)


def generate_review_copy(client, title, sample, n=3):
    """根据宝贝标题 / 文案范例,生成 n 条真实、无 AI 味的买家秀文案(每条 80-160 字)。
    文案规则来自 35,000 条真实评价分析,见上方 REVIEW_MIX 注释。"""
    sys_prompt = (
        "你是一个真实的珠宝网店买家,刚收到并戴上了首饰,在淘宝/小红书写真实评价。"
        "写作要求:口语化、有具体生活细节、像真人随手写的;绝对不能有 AI 腔和模板感,"
        "不要华丽排比、不要堆砌形容词、不要每条开头都一样。可以有一点点小口误感、小语气词,自然真实。"
        "你的目标不是写得好看,而是精准命中买家的决策点,同时提前对齐预期——"
        "夸大到无法兑现只会换来中差评。"
    )
    user_prompt = (
        f"【宝贝标题】{title or '(未提供)'}\n"
        ...
        "【主题配比 · 必须按这个分,不要全写「很闪很喜欢」】\n"
        f"{_mix_for(n)}\n"
        "(依据:这些是 3 万条真实评价里买家自己投票最多的点,顺序就是他们在乎的顺序)\n\n"
        "【必须做的两条预期管理 · 这是降退货的关键】\n"
        "1. 至少 1 条要说清光线:钻石在室内灯光下和阳光下最闪,阴天户外散射光下会显得平一些。"
        "真实中评里就有「自然光下不闪」这种落差,提前说明反而显得诚实可信。\n"
        "2. 至少 1 条要主动帮人判断大小,倾向于劝小:比如「50 分在手上已经很显了,1 克拉可能偏夸张」"
        "或提到客服免费寄测量圈、圈号一次到位。尺寸落差是买家差评第一名。\n\n"
        f"【追评】{max(1, n // 3)} 条要带追评(格式:正文后面另起一行写「追评:……」),"
        "追评里必须提到戴了一段时间「没掉钻」「没变色/没发黄」——这两点是买家最担心的长期问题。\n\n"
        "【绝对不能写】\n"
        "- 保值、升值、高价回收(这是这个类目最大的差评来源,一旦被识破就是「欺骗消费者」级投诉)\n"
        "- 和天然钻一模一样、完全无法区分(仪器能区分成因;只能写「肉眼和日常佩戴上没有区别」)\n"
        "- 全网最低/最闪/第一(违反广告法)\n"
        "- 不要编造具体的证书类型(标题里没写就别提证书机构名)\n\n"
        "每条可带 0-1 个 emoji(也可不带)。每条的句式、长短、语气要有明显差异,"
        "读起来像 N 个不同的人写的。\n"
...
def render_buyer_show(api_key):
    ...
    with st.expander("📝 顺便生成买家秀文案(真实、无 AI 味,80-160 字 · 一组一套)"):
        ...
        copy_n = st.slider("生成几套文案", 1, 10, value=int(n_scenes), key="bs_copy_n",
                           help="默认 = 场景数量(几组买家秀就配几套文案),也可手动改。"
                                "选 10 条时正好是完整配比:闪度3/尺寸2/做工2/性价比1/场景1/服务1")
