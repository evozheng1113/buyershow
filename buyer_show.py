...
# ---- 真人佩戴(10 张正式 + 备用) ----
# 融入办公桌、凌乱梳妆桌、副驾、木桌(钥匙+咖啡)、拆开的快递盒+气泡膜等生活化背景。
WORN_FIXED = [
    "场景:办公桌前,背景有电脑显示器、键盘、水杯和散落的文件。手自然搭在桌上,佩戴部位特写,脸在画框外。光线是室内偏冷白光,随手拍。",
    "场景:凌乱的梳妆桌前,画面略带模糊。桌上散落化妆品、棉签、小首饰和瓶瓶罐罐,低头露出佩戴部位、头发遮住脸。光线是暖黄灯,偏暗有噪点。",
    "场景:汽车副驾座位上,系着安全带,车窗外是自然光和虚化的街景。侧脸只露半张,佩戴部位入镜。光线略逆光,真实不修。",
    "场景:木桌前,桌上随手放着一串钥匙和一杯咖啡。手搭在桌边,佩戴部位特写,脸不入镜。光线是窗边自然光,柔和。",
    "场景:桌上有刚拆开的快递盒、气泡膜和几样零散小物。像刚收到货戴上,佩戴部位特写,脸在画框外。光线是室内灯偏黄,随手一拍。",
    "场景:午后靠窗的咖啡馆,木桌上一杯拿铁。侧身低头,长发垂下遮住大半张脸,佩戴部位特写。光线是午后斜射自然光,略微过曝。",
    "场景:周末家里的沙发上,旁边是抱枕和毛毯。慵懒侧靠,手自然搭着,脸只露一点点,镜头对着佩戴部位。光线是室内暖光,偏暗。",
...
def build_scene_pool(n=None, rng=None, jewelry_type: str = "自动判断"):
    ...
    每个场景带 ref 字段:'wearing'(用佩戴图) / 'box_black' / 'box_burgundy' / 'none'。
    jewelry_type 决定佩戴部位。n / rng 参数保留以兼容旧调用,不再使用。"""
    directive = JEWELRY_TYPES.get(jewelry_type, JEWELRY_TYPES["自动判断"])

    plan = (
        [("worn", "wearing", WORN_TPL.format(body=b)) for b in WORN_FIXED[:N_WORN]]
        + [("held", "wearing", HELD_TPL.format(body=b)) for b in HELD_FIXED[:N_HELD]]
